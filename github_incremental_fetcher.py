        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if stargazers and not is_recent_data(stargazers, 'stargazers'):
                    print(f"Non-200 status ({response.status_code}) but data is not recent. Sleeping 10 seconds...")
                    time.sleep(10)
                    continue
                else:
            ...
            print(f"Fetched stargazers page {page}, total stargazers so far: {len(stargazers)}")
...
def merge_data_by_date(existing_data, new_stars_by_date, new_forks_by_date, cutoff_date):
    ...
    filtered_stars = {date: count for date, count in existing_stars.items() if date < cutoff_date}
    filtered_forks = {date: count for date, count in existing_forks.items() if date < cutoff_date}
...
def main():
    ...
    parser.add_argument('--last-stargazers-page', type=int, help='Last stargazers page number for incremental update')
    parser.add_argument('--last-forks-page', type=int, help='Last forks page number for incremental update')
    ...
    try:
        ...
        output_dir = "repository_informations"
        ...
        if args.last_stargazers_page:
            ...
            print(f"Starting stargazers fetch from page {stargazers_start_page} (last page was {args.last_stargazers_page})")
        
        if args.last_forks_page:
            forks_start_page = max(1, args.last_forks_page - 2)
            print(f"Starting forks fetch from page {forks_start_page} (last page was {args.last_forks_page})")
        ...
        if existing_data and (args.last_stargazers_page or args.last_forks_page):
            ...
            if all_dates:
                ...
                cutoff_date_obj = datetime.strptime(last_date, '%Y-%m-%d') + timedelta(days=1)
                cutoff_date = cutoff_date_obj.strftime('%Y-%m-%d')
                print(f"Incremental update: will merge data from {cutoff_date} onwards")
        
        # Fetch stargazers
        print("\n=== Fetching Stargazers ===")
        stargazers, last_stargazers_page = fetch_stargazers(owner, repo, token, stargazers_start_page)
        ...
        if cutoff_date:
            ...
            print(f"Merged data: keeping existing data before {cutoff_date}, updating from {cutoff_date} onwards")
        else:
        ...
        output_data = OrderedDict([
            ...
            ("stars_by_date", OrderedDict(sorted(stars_by_date.items())) if stars_by_date else OrderedDict()),
            ("forks_by_date", OrderedDict(sorted(forks_by_date.items())) if forks_by_date else OrderedDict())
        ])
        ...
        print(f"Total forks: {sum(forks_by_date.values()) if forks_by_date else len(forks)}")
        print(f"Last stargazers page: {last_stargazers_page}")
        print(f"Last forks page: {last_forks_page}")
        if stars_by_date:
            print(f"Stars date range: {min(stars_by_date.keys())} to {max(stars_by_date.keys())}")
            print(f"Days with stars: {len(stars_by_date)}")
        if forks_by_date:
            print(f"Forks date range: {min(forks_by_date.keys())} to {max(forks_by_date.keys())}")
